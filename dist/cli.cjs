...
function auditProject(targetDir, options = {}) {
  ...
  const hasV8Entries = files.some((f) => {
    const rel = path__default.default.relative(resolvedTarget, f);
  ...
  for (const filePath of files) {
    ...
    if (hasV8Entries && !relPath.includes(path__default.default.sep)) {
      const base = path__default.default.basename(relPath);
    ...
    if (!inTest && !inBuildOrTool) {
      const inClientDir = relPath.split(path__default.default.sep).includes("client");
      const inServerDir = relPath.split(path__default.default.sep).includes("server");
  ...
  try {
    const manifestPath = path__default.default.join(resolvedTarget, "paseo-plugin.json");
    if (fs2__default.default.existsSync(manifestPath)) {
      const manifest = JSON.parse(fs2__default.default.readFileSync(manifestPath, "utf-8"));
  ...
  if (helperClientUsed && !initCalled) {
    const entry = files.map((f) => path__default.default.relative(resolvedTarget, f)).find((rel) => rel === "index.client.tsx" || rel === "index.client.ts") ?? "index.client.tsx";
    pushIssue(
      "missing-client-init",
      entry,
      1,
